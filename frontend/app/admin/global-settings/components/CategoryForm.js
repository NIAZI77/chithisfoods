...
const CategoryForm = ({
  ...
  const updateSubcategory = (index, value) => {
    if (!isValidInput(value)) {
      toast.error(
        "Only letters, spaces, and ampersands (&) are allowed in category names"
      );
      return;
    }
    setFormData((prev) => ({
  ...
  const addSubSubcategory = (subcategoryIndex) => {
    ...
    const hasEmptySubSubcategories = currentSubcategory.subSubcategories.some(
      (ss) => !ss.trim()
    );

    if (hasEmptySubSubcategories) {
      toast.error(
        "Please fill in all existing sub-subcategories first before adding new ones"
      );
      return;
    }
  ...
  const updateSubSubcategory = (
    ...
    if (!isValidInput(value)) {
      toast.error(
        "Only letters, spaces, and ampersands (&) are allowed in subcategory names"
      );
      return;
    }
    setFormData((prev) => ({
  ...
  const handleKeyPress = (e, subcategoryIndex, subSubcategoryIndex) => {
    if (e.key === "Enter") {
      ...
      const currentSubcategory = formData.subcategories[subcategoryIndex];
      const currentValue =
        currentSubcategory.subSubcategories[subSubcategoryIndex].trim();
      // If not the last sub-subcategory, move to the next input
      if (
        subSubcategoryIndex <
        currentSubcategory.subSubcategories.length - 1
      ) {
        if (!currentValue) {
        ...
        setTimeout(() => {
          ...
          const subInputs = Array.from(allInputs).filter(
            (input) => input.placeholder === "Enter sub-subcategory"
          );
          const baseIndex =
            subInputs.length * subcategoryIndex + subSubcategoryIndex;
        ...
        const hasEmpty = currentSubcategory.subSubcategories.some(
          (ss) => !ss.trim()
        );
  ...
  const handleSubmit = (e) => {
    ...
    if (hasEmptySubcategories) {
      toast.error(
        "Please fill in all subcategory and sub-subcategory fields to continue"
      );
      return;
    }
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              {formData.subcategories.map((subcategory, index) => (
                <div
                  key={index}
                  className="space-y-2 p-4 border border-pink-100 rounded-lg hover:border-pink-200 transition-colors"
                >
