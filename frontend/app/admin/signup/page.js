    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            if (!checkAdminResponse.ok) {
                throw new Error(`Failed to check existing admin users: ${checkAdminResponse.status}`);
            }

            const adminCheckData = await checkAdminResponse.json();
            const existingAdmins = Array.isArray(adminCheckData) ? adminCheckData : (adminCheckData.data || []);
            ...
            const registerResponse = await fetch(
                `${process.env.NEXT_PUBLIC_STRAPI_HOST}/api/auth/local/register`,
                {
                    ...
                    body: JSON.stringify({
                        username: email.split('@')[0] + Math.floor(Math.random() * 10000000),
                        email,
                        password,
                    }),
                }
            ...
            if (!registerResponse.ok) {
                if (registerData.error?.message?.includes("already exists")) {
                    ...
                    toast.error(registerData.error?.message || "Failed to create user account");
                }
                return;
            }
            ...
            if (updateResponse.ok) {
                if (admintype === "main") {
                    toast.success(`Main Admin account created successfully! now you can access all features`);
                } else {
                    toast.success(`Admin Account created successfully! Please wait for main admin to verify.`);
                }
                setTimeout(() => router.push("/admin/login"), 2000);
            } else {
                toast.error("User created but failed to assign admin role. Please contact support.");
            }
        } catch (error) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 lg:p-8 bg-white">
            ...
            <div className="bg-white p-8 rounded-lg shadow-sm w-full max-w-md border border-gray-100">
                <h2 className="text-2xl font-bold text-center mb-8 text-gray-800">Admin Sign Up</h2>
