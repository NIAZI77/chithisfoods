...
import { STATUS_STYLES, PAYMENT_STATUS_STYLES, BUTTON_STYLES, ACTION_STATUS_STYLES } from "../constants";
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Date</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Customer</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Vendor</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Order Status</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Payment Status</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Vendor Payment</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Subtotal</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Delivery Fee</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Tax</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Total</th>
              <th className="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {orders.map((order) => (
              <tr key={order.documentId} className="bg-white hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-left">{order.customerName}</td>
                <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-left">@{order.vendorUsername}</td>
                ...
                <td className="px-4 py-3 text-sm text-center text-gray-900 whitespace-nowrap">${order.subtotal?.toFixed(2) || '0.00'}</td>
                <td className="px-4 py-3 text-sm text-center text-gray-900 whitespace-nowrap">${order.deliveryFee?.toFixed(2) || '0.00'}</td>
                <td className="px-4 py-3 text-sm text-center text-gray-900 whitespace-nowrap">${order.tax?.toFixed(2) || '0.00'}</td>
                <td className="px-4 py-3 text-sm text-center font-medium text-gray-900 whitespace-nowrap">${order.totalAmount.toFixed(2)}</td>
                <td className="px-4 py-3 text-sm text-center space-y-2 whitespace-nowrap">
                  <div className="flex flex-col gap-2 items-center">
                    {order.orderStatus === "delivered" && order.vendor_payment === "unpaid" && (
                      <button
                        onClick={() => onProcessVendorPayment(order.documentId)}
                        disabled={processingPayments[order.documentId] || !vendorPaypalEmails[order.vendorId]}
                        className={`${BUTTON_STYLES.payVendor} ${
                          (processingPayments[order.documentId] || !vendorPaypalEmails[order.vendorId]) 
                          ? 'opacity-50 cursor-not-allowed' 
                          : ''
                        }`}
                        title={!vendorPaypalEmails[order.vendorId] ? "Vendor PayPal email not provided" : ""}
                      >
