...
const UserDetailsModal = ({ user, isOpen, onClose, onBlockUser, isLoading, onVerifyVendor }) => {
        ...
        if (isOpen && user && user.email) {
            ...
            const fetchVendorData = async (email) => {
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(data.error?.message || "Unable to retrieve vendor information.");
                    }
    ...
    return (
        ...
        >
            ...
            >
                ...
                <h2 className="text-2xl font-extrabold text-pink-600 mb-6 text-left tracking-tight">User Details</h2>
                {isLoading ? (
                    <div className="flex justify-center items-center min-h-[120px]"><Spinner /></div>
                ) : !user ? (
                    ...
                    <>
                        ...
                        <div className="mt-2 flex flex-col gap-2">
                            {vendorLoading && <div className="text-sm text-gray-400 flex items-center gap-2"><Spinner size={16}/> Checking vendor status...</div>}
                            {vendorError && <div className="text-sm text-red-500">{vendorError}</div>}
                            {vendorData && (
                                <button
