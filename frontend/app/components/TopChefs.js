...
export default function TopChefs({ zipcode }) {
  ...
  const fetchTopVendorsByZipcode = async (zipcode) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_STRAPI_HOST}/api/vendors?filters[serviceArea][$containsi]=${zipcode}&populate=*&sort[0]=weeklyItemsSold:desc&pagination[pageSize]=6`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        toast.error(
          "We're having trouble loading top chefs right now. Please try again later."
        );
        setTopVendors([]);
      }
    } catch (error) {
  ...
  return (
    <div className="md:w-[80%] w-full mx-auto p-2">
      <h2 className="md:text-2xl text-xl font-bold mb-4">Top Chefs</h2>
